  ...
  "topics": [
    ...
    if unassigned and topic_names_ko:
        ...
        if to_embed:
            ...
            topic_embeddings, t1 = embed(client, topic_names_ko)
            comment_embeddings, t2 = embed(client, [c["text"] for c in to_embed])
            tokens["embedding"] += t1 + t2

            for i, comment in enumerate(to_embed):
                sims = [cosine_sim(comment_embeddings[i], te) for te in topic_embeddings]
